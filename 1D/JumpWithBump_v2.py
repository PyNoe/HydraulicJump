...
def topo(x):
    return 0.5 * np.exp(-(x - 4.0)**2 / 1.0**2)
...
def update_timestep(U, flux_func, dt):
    ...
    # BC droite : on peut faire flux nul ou "copy"
...
while t < tmax:
    ...
    # calcul dt en CFL
    h_loc = U_rus_current[0,:]  # par ex on se base sur la solution Rusanov
...
all_hb = np.concatenate([rus_hb, hll_hb, roe_hb], axis=0)  # shape=(3*n_snap, Nx)
...
ani.save("SimulationExp1.mp4", fps=60)

plt.tight_layout()
plt.show()

plt.plot(x_grid, rus_hb[0,:], label='Rusanov', color='blue')
