...
def flux_roe(U_L, U_R):    #Simplifié ^^
    F_L = compute_flux(U_L.reshape(2,1))[:,0]
    F_R = compute_flux(U_R.reshape(2,1))[:,0]
    hL, huL = U_L
    hR, huR = U_R
...
while t < tmax:
    ...
    # calcul dt en CFL
    h_loc = U_rus_current[0,:]  # On se base sur la solution Rusanov
...
U_rus_data  = np.array(U_rus_data) 
U_hll_data  = np.array(U_hll_data)
U_roe_data  = np.array(U_roe_data)

# Fonctions pour avoir une bonne size des axes
...
rus_hb = get_h_plus_b(U_rus_data)  
